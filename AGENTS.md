# sharehtml - Codex Configuration

This file configures Codex behavior for the sharehtml project.
...
- Keep `NODE_AUTH_TOKEN: ${{ secrets.NPM_TOKEN }}` on the publish workflow's `npm publish` step; do not add setup-node cache inputs to release builds.
